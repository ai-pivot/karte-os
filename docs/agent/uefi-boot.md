...
3. **Page tables**: Set up PML4 with identity map (0-8GB) + direct map (high-half 0-2GB). If GOP framebuffer is above 8GB, dynamically add PML4 entry using `PDP_HIGH`
