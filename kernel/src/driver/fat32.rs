...
mod x86_64_impl {
    ...
    fn block_capacity() -> u64 {
        crate::driver::ahci::capacity()
            .or_else(|| {
                // Try VirtIO
                unsafe { crate::arch::virtio_blk::capacity() }
            })
            .unwrap_or(0)
    }
    ...
    impl Fat32Storage {
        pub fn new() -> Self {
            let cap = block_capacity() * SECTOR_SIZE;
            Self {
                position: 0,
                capacity_bytes: cap,
            }
    ...
    impl Read for Fat32Storage {
        fn read(&mut self, buf: &mut [u8]) -> Result<usize, Self::Error> {
            ...
            while total_read < buf.len() {
                let byte_offset = offset_in_first_sector + total_read;
                let sector_idx = base_sector + byte_offset / 512;
                block_read(sector_idx, &mut sector_buf).map_err(|_| StorageError("read failed"))?;
    ...
    impl Write for Fat32Storage {
        fn write(&mut self, buf: &[u8]) -> Result<usize, Self::Error> {
            ...
            while total_written < buf.len() {
                ...
                block_read(sector_idx, &mut sector_buf)
                    .map_err(|_| StorageError("read-modify-write: read failed"))?;
                ...
                block_write(sector_idx, &sector_buf)
                    .map_err(|_| StorageError("read-modify-write: write failed"))?;
                total_written += to_copy;
            }
    ...
    pub fn init() -> Result<(), &'static str> {
        ...
        match FileSystem::new(storage, FsOptions::new()) {
            Ok(fs) => {
                crate::console_println!("[fat32] Filesystem mounted successfully");
                *FAT32_FS.lock() = Some(fs);
                FAT32_AVAILABLE.store(true, Ordering::SeqCst);
                return Ok(());
            }
            Err(_) => {
                crate::console_println!("[fat32] No filesystem found, formatting...");
        ...
        crate::console_println!("[fat32] Formatted, mounting...");
        let storage = Fat32Storage::new();
        match FileSystem::new(storage, FsOptions::new()) {
            Ok(fs) => {
                crate::console_println!("[fat32] Filesystem formatted and mounted");
    ...
    pub fn inject_file(_name: &str, _data: &[u8]) -> Result<(), &'static str> {
        // File injection into FAT32 for x86_64 — not yet implemented
        Err("FAT32 inject not implemented on x86_64")
    }
    ...
    pub fn write_file(name: &str, data: &[u8]) -> Result<(), &'static str> {
        ...
        let mut file = root
            .create_file(name)
            .map_err(|_| "FAT32 create file failed")?;
        file.write_all(data).map_err(|_| "FAT32 write failed")?;
    ...
    pub fn list_root() -> Vec<(String, usize)> {
        ...
        for entry in root.iter() {
            if let Ok(e) = entry {
                ...
                if name != "." && name != ".." {
                    result.push((String::from(name), e.len() as usize));
                }
            }
        }
        result
