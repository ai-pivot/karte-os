...
mod x86_64_stub {
    ...
    pub fn init() -> Result<(), &'static str> {
        Err("ext4 not available on x86_64 (no block device)")
    }
    ...
    pub fn lookup_path(_name: &str) -> Option<u64> {
        None
    }

    pub fn metadata_of(_inode: u64) -> Option<crate::driver::vfs::VfsMetadata> {
        None
    }

    pub fn write_file(_name: &str, _data: &[u8]) -> Result<(), &'static str> {
        Err("ext4 not available on x86_64")
    }

    pub fn list_directory(_path: &str) -> Vec<(String, usize)> {
        Vec::new()
    }

    pub fn create_directory(_name: &str) -> Result<(), &'static str> {
        Err("ext4 not available on x86_64")
    }

    pub fn delete_file(_name: &str) -> Result<(), &'static str> {
        Err("ext4 not available on x86_64")
    }

    #[cfg(feature = "test_mode")]
    pub fn run_tests() {
        // No ext4 tests on x86_64
    }
}
