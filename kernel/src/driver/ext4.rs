...
/// Checkpoint SQLite WAL: merge valid WAL frames into the main db file.
/// Called on process exit to ensure data persists across restarts.
/// This bypasses SQLite's own checkpoint (which fails due to WAL extent
/// tree corruption and SHM being all-zeros).
#[cfg(target_arch = "x86_64")]
pub fn wal_checkpoint(db_path: &str) {
    let wal_path: alloc::string::String = alloc::format!("{}-wal", db_path);
    let shm_path: alloc::string::String = alloc::format!("{}-shm", db_path);
    let wal_path = wal_path.as_str();
    let shm_path = shm_path.as_str();

    // Read WAL file
    let wal_data = match read_file(&wal_path) {
        Some(d) if d.len() >= 32 => d,
        _ => return, // No WAL or too small
    };

    // Parse WAL header (big-endian fields)
    ...
    let salt1 = &wal_data[16..20];
    let salt2 = &wal_data[20..24];

    let frame_size = 24 + page_size;
    let total_frames = (wal_data.len() - 32) / frame_size;

    // Scan frames, find the last commit frame
    let mut last_commit_frame: i32 = -1;
    for i in 0..total_frames {
        let frame_off = 32 + i * frame_size;
        if frame_off + 24 > wal_data.len() {
            break;
        }
        let db_size = u32::from_be_bytes([
            wal_data[frame_off + 4],
            wal_data[frame_off + 5],
            wal_data[frame_off + 6],
            wal_data[frame_off + 7],
        ]);
        let frame_salt1 = &wal_data[frame_off + 8..frame_off + 12];
        let frame_salt2 = &wal_data[frame_off + 12..frame_off + 16];
        ...
        if db_size > 0 {
            last_commit_frame = i as i32;
        }
    }

    if last_commit_frame < 0 {
        return; // No commit frames
    }

    // Apply frames up to last commit to main db
    // Build a map of page_number -> page_data (last write wins)
    use alloc::collections::BTreeMap;
    let mut pages: BTreeMap<u32, &[u8]> = BTreeMap::new();

    for i in 0..=last_commit_frame as usize {
        let frame_off = 32 + i * frame_size;
        let pgno = u32::from_be_bytes([
            wal_data[frame_off],
            wal_data[frame_off + 1],
            wal_data[frame_off + 2],
            wal_data[frame_off + 3],
        ]);
        let page_data = &wal_data[frame_off + 24..frame_off + 24 + page_size];
        pages.insert(pgno, page_data);
    }

    // Get the db_size from the last commit frame
    let commit_frame_off = 32 + (last_commit_frame as usize) * frame_size;
    let db_size_pages = u32::from_be_bytes([
        wal_data[commit_frame_off + 4],
        wal_data[commit_frame_off + 5],
        wal_data[commit_frame_off + 6],
        wal_data[commit_frame_off + 7],
    ]);

    crate::console_println!(
        "[wal-checkpoint] {} pages, {} frames, db_size={}",
        pages.len(),
        last_commit_frame + 1,
        db_size_pages
    );

    // Write each page to the main db file
    // We need the db inode for write_file_at_offset
    let db_inode = {
        let guard = EXT4_FS.lock();
        guard
            .as_ref()
            .and_then(|fs| fs.lookup(ROOT_INODE as u64, db_path).ok())
    };
    ...
    let db_inode = db_inode.unwrap() as u32;

    // Write pages sequentially
    for (pgno, data) in &pages {
        let offset = (*pgno as usize - 1) * page_size;
        let _ = write_file_at_offset(db_inode, offset, data);
    }

    // Update db file size
    let new_size = db_size_pages as usize * page_size;
    let _ = set_file_size(db_inode, new_size);

    // Truncate WAL to 0 (or delete it)
    let _ = delete_file(&wal_path);

    // Delete SHM (force clean recovery on next open)
    let _ = delete_file(&shm_path);

    crate::console_println!("[wal-checkpoint] done, db size={} bytes", new_size);
}
...
/// Set file size to a specific value (truncate or extend).
#[cfg(target_arch = "x86_64")]
pub fn set_file_size(inode: u32, size: usize) -> Result<(), &'static str> {
    if !has_ext4() {
        return Err("ext4 not initialized");
    }
    let mut guard = EXT4_FS.lock();
    let fs = guard.as_mut().ok_or("ext4 not initialized")?;
    fs.set_file_size(inode as u64, size)
        .map_err(|_| "ext4 set_file_size failed")
}
