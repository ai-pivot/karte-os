...
- **x86_64 page fault lazy allocation**: Page fault handler checks if fault address is in user heap area (USER_HEAP_BASE..USER_HEAP_LIMIT). If so, allocates a physical frame, maps it into the user page table, and retries. Terminates user process on fatal page faults.
