...
- **x86_64 PF handler vma_file_info MUST use page_addr (aligned), NOT fault_addr_val**: File-backed mmap page faults must pass `page_addr` (CR2 & ~0xFFF) to `vma_file_info`, never the raw CR2 value. `vma_file_info` computes `file_off = vma.file_offset + (addr - vma.start)` — using an unaligned address adds the sub-page byte offset, loading the entire 4096-byte page from a shifted file position. Symptom: SQLite SHM mmap reads return shifted content → salt mismatch → WAL silently ignored → session data appears lost despite correct disk bytes.
