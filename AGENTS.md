...
- **x86_64 SYSCALL uses TSS.RSP0**: `syscall_entry` reads per-task kernel stack from TSS.RSP0 (via `TSS_RSP0_ADDR` pointer), NOT from a global `SYSCALL_KSP`. This is critical because Timer ISR can preempt during CPL=0 SYSCALL execution (IST=0 → no stack switch). Per-task stacks prevent corruption when `__switch()` saves/restores SPs. `TSS_RSP0_ADDR` is a `#[unsafe(no_mangle)]` global pointing to the TSS RSP0 field.
