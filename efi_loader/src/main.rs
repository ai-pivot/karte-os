...
pub extern "C" fn efi_main(image_handle: EfiHandle, system_table: *const EfiSystemTable) -> ! {
    ...
    // Jump to _start64 at its IDENTITY (physical) address, NOT high-half.
    // _start64 is in .text.boot which the linker places at low VMA
    // (0x1001D8), while .bss / .data are at high-half VMA.  The
    // RIP-relative lea instructions inside _start64 use 32-bit signed
    // displacements — these overflow when %rip is at a high VMA but
    // the linker computed them assuming a low VMA.  Entering at the
    // identity address (like GRUB does) avoids this.
    let entry_identity = (KERNEL_PHYS_BASE + start64_offset) as u64;
